...
class Canvas {
  ...
  /** Total OGL world units needed to scroll through all cards once */
  getScrollTotal(): number {
    if (!this.medias || this.medias.length === 0) return 0;
    // heightTotal is the same on all Media instances; grab from first
    return this.medias[0].heightTotal;
  }

  /** Directly set scroll target without triggering React re-render */
...
export type FlyingPostersHandle = {
  instance: Canvas | null;
  setScroll: (value: number) => void;
  getScrollTotal: () => number;
};

const FlyingPosters = forwardRef<FlyingPostersHandle, FlyingPostersProps>(
  (
    {
      items = [],
      planeWidth = 320,
    ...
    useImperativeHandle(ref, () => ({
      ...
      setScroll: (value: number) => instanceRef.current?.setScroll(value),
      getScrollTotal: () => instanceRef.current?.getScrollTotal() ?? 0,
    }));
    ...
    useEffect(() => {
      ...
      if (!disableInternalScrolling) {
        instanceRef.current.addEventListeners();
      }
    ...
    useEffect(() => {
      if (!canvasRef.current || disableInternalScrolling) return;

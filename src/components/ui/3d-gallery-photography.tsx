...
import styles from "./3d-gallery-photography.module.css";
...
interface InfiniteGalleryProps {
  ...
  /** If true, the gallery will not listen to internal wheel/keydown events */
  isScrollControlled?: boolean;
  /** External scroll progress (0-1) to control the gallery depth */
...
function GalleryScene({
  ...
  const [autoPlay, setAutoPlay] = useState(!isScrollControlled);
  ...
  useFrame((state, delta) => {
    // Apply auto-play or external scroll progress
    if (isScrollControlled) {
      // Map 0-1 progress to a sensible scroll velocity or direct offset
      // We'll calculate a virtual velocity based on progress change to keep the existing wrapping logic
      const targetVelocity = 0; // In controlled mode, we might want to directly set Z instead
      // But for simplicity of keeping the wrapping logic, let's just use it to drive the Z
    } else if (autoPlay) {
      setScrollVelocity((prev) => prev + 0.3 * delta);
    }

    // Damping only if not controlled
    if (!isScrollControlled) {
    ...
    planesData.current.forEach((plane, i) => {
      let newZ;
      if (isScrollControlled) {
        // Each 0.1 progress moves through roughly one image spacing
        // Adjust 10.0 to control how "fast" the gallery moves relative to scroll
        const offset = scrollProgress * depthRange * 2;
        // Initial distribution + offset
        const initialZ =
          ((depthRange / Math.max(visibleCount, 1)) * i) % depthRange;
        newZ = (initialZ + offset) % depthRange;
      } else {
        newZ = plane.z + scrollVelocity * delta * 10;
      }

...
const TextPressure: React.FC<TextPressureProps> = ({
  ...
  }, [chars.length, minFontSize, scale, fixedFontSize]);
    ...
    const animate = () => {
      ...
      if (titleRef.current) {
        ...
        spansRef.current.forEach((span) => {
          ...
          const wdthMax = 200;
          const wdth = width
            ? Math.round(wdthMin + (wdthMax - wdthMin) * proximity)
            : 100;

          const italVal = italic ? proximity.toFixed(2) : "0";
          const alphaVal = alpha ? proximity.toFixed(2) : "1";
